...
async function runScenario(browser, index) {
  ...
  const getChoices = () => page.$$eval('[data-choice]', (elements) => elements.map((element) => ({ id: element.getAttribute('data-choice') ?? '' })));
    ...
    if (choices.length > 0) {
      const selected = choices
        .map((choice) => {
          ...
          if (/dash/i.test(choice.id) && index === 2) score += 0.6;
          return { choice, score };
        })
...
const runs = [];
for (let index = 1; index <= 3; index += 1) runs.push(await runScenario(browser, index));
await browser.close();
const aggregate = {
