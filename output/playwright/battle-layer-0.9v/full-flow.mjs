...
const outputDir = 'E:/codex/unity-learning/output/playwright/battle-layer-0.9v';
...
async function main() {
  ...
  const chooseFromPanel = async (panel) => {
    ...
    if (isNodePanel) {
      ...
      target =
        ...
        panel.choices.find((choice) => choice.text.includes('紫') || choice.text.includes('金')) ||
        panel.choices[0];
    } else {
      target =
        panel.choices.find((choice) => choice.text.includes('窗口') || choice.text.includes('样本') || choice.text.includes('承压')) ||
        panel.choices.find((choice) => !choice.type.includes('事件')) ||
        panel.choices[0];
    }
  ...
  for (let step = 0; step < 56; step += 1) {
    ...
    if (hudCount > 0) {
      ...
      if (hudText.includes('Boss载体') || hudText.includes('精英战') || hudText.includes('生存战') || hudText.includes('普通战')) {
        summary.battleHudSeen = true;
      }
      if (hudText.includes('Boss载体')) {
